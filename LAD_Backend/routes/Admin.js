...
router.get('/getReviewTasks',(req,res)=>{
  ...
  ReviewTask.find().populate('synopsis').populate('reviewer').populate('comments')
  .then(programsList=>{
    res.status(200).json({
...
router.post("/registerFaculty", async (req, res) => {
  ...
  let faculty2 = await Faculty.findOne({ email: req.body.email });
    ...
    .then((result) => {
      ...
      token
        ...
        .then((result2) => {
          ...
          return res
            .status(200)
            .json({ message: "Faculty Registered Success!!!" });

...
router.post("/getdepartmentId", (req, res) => {
  Department.find({ name: req.body.name })
    .exec()
    .then((result) => res.send(result[0]._id));
});
...
router.post("/getAllD", (req, res) => {
  Department.find()
    .exec()
    .then((result) => res.send(result));
});

router.post("/getAllP", (req, res) => {
  Program.find()
    .exec()
    .then((result) => res.send(result));
});
router.post("/getdepartmentname", (req, res) => {
  Department.find({ _id: req.body.id })
    .exec()
    .then((result) => res.send(result[0].name));
});

router.post("/getProgramId", (req, res) => {
  Program.find({ name: req.body.name })
    .exec()
    .then((result) => res.send(result[0]._id));
});

router.post("/getProgramName", (req, res) => {
  Program.find({ _id: req.body.id })
    .exec()
    .then((result) => {
      console.log(result);
...
router.post("/getFaculity", (req, res) => {
  Faculty.find({ _id: req.body.id })
    .exec()
    .then((result) => res.send(result[0]));
});

router.post("/getFaculityPro", (req, res) => {
  Faculty.find({ program: req.body.name })
    .exec()
    .then((result) => res.send(result[0]._id));
});
      ...
      bcrypt.compare(req.body.password, hash, function (err, result) {
        if (result) {
          const token = jwt.sign(
            {
              first: faculty[0].fname,
              last: faculty[0].lname,
              email: faculty[0].email,
              department: faculty[0].department,
              program: faculty[0].program,
...
router.get("/getStudent", (req, res) => {
  ...
  Student.find({ program: req.body.programId })
    .then((student) => {
      console.log(student);
      res.status(200).json({
        data: student,
...
router.post("/getStudents", (req, res) => {
  console.log(req.body.id);
  Synopsis.find({ supervisor: req.body.id })
    .populate("student")
    .then((synopsis) => {
      var synopsisdata = [];
...
router.post("/getReviewTasks", (req, res) => {
  console.log(req.body.id);
  ReviewTask.find({ reviewer: req.body.id })
    .populate("synopsis")
    .populate("commenents")
    .then((programsList) => {
      res.status(200).json({

...
const { Feedbackss } = require("../models/Feedback");
...
router.post("/givefeedback", async (req, res) => {
  let feedback = new Feedbackss({
    programId: req.body.programId,
    faculityId: req.body.faculityId,
...
router.post("/getFeedback", async (req, res) => {
  await Feedbackss.find({ programId: req.body.id })
    .then((result) => {
      res.send(result);
    })
    .catch((e) => {
...
router.post("/getStudents", async (req, res) => {
  await Student.find({ program: req.body.programid })
    .exec()
    .then((result) => {
      res.send(result);
    });
...
router.post("/getSID", async (req, res) => {
  await Student.find({ _id: req.body.id })
    .exec()
    .then((result) => {
      console.log(result);
      res.send(result[0].program);
    });
});
